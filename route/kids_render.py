...
async def analyze_image_with_gpt_vision(img_bytes: bytes) -> str:
    """GPT-4o Vision을 사용하여 이미지를 분석하고 Tripo용 프롬프트 생성"""
...
1. Identify the main subject (e.g., a lion, a car, a monster).
2. Identify the primary colors used. If colors are unclear, use popular/vibrant colors suitable for a toy.
3. Describe it as a 'volumetric, cute, high-quality 3D plastic toy'.
4. Ensure the style is 'smooth 3D rendering' with no background clutter.
5. Create a CONCISE 3D generation prompt for Tripo AI. 
Example Output: 'A cute volumetric 3D plastic toy lion, yellow body with brown mane, vibrant colors, smooth glossy finish, studio lighting'
...
# Gemini logic removed - superseded by GPT Vision
# def _render_one_image_sync...
# async def render_one_image_async ...
...
async def process_kids_request_internal(
    ...
    try:
        ...
        with anyio.fail_after(KIDS_TOTAL_TIMEOUT_SEC):
            ...
            # -----------------
            # 1) GPT-4o Vision 분석 및 텍스트 프롬프트 생성
            # -----------------
            step_start = time.time()
            log(f"📌 [STEP 1/4] GPT-4o Vision 이미지 분석 시작...")
            gpt_prompt = await analyze_image_with_gpt_vision(img_bytes)
            log(f"✅ [STEP 1/4] 분석 완료 | Prompt: {gpt_prompt} | {time.time()-step_start:.2f}s")

            # -----------------
            # 2) Tripo 3D (텍스트 → 3D 모델 생성 : Method B)
            # -----------------
            step_start = time.time()
            log(f"📌 [STEP 2/4] Tripo 3D 모델 생성 시작 (text-to-model)... (timeout={TRIPO_WAIT_TIMEOUT_SEC}s)")

            # ✅ Backend에 stage 업데이트
            await update_job_stage(job_id, "THREE_D_PREVIEW")

            async with TripoClient(api_key=TRIPO_API_KEY) as client:
                # ✅ text_to_model: GPT가 생성한 프롬프트로 3D 생성
                task_id = await client.text_to_model(prompt=gpt_prompt)
            ...
            # (나머지 로직은 동일)
            # 입체화된 모델을 나타내기 위해 'corrected' 대신 'analzed' 등의 용어를 쓰지만 
            # 변수명 호환을 위해 유지하거나 내부적으로만 처리
            corrected_url = source_image_url # 원본 이미지를 그대로 보여줌 (분석은 이미 했으므로)
