...
# -----------------------------------------------------------------------------
# 6.5 1x1 브릭 자동 제거 (안정성 향상)
# -----------------------------------------------------------------------------
# 1x1 브릭(3005.dat)은 연결이 불안정하므로 자동으로 제거
BRICK_1X1_PARTS = {"3005.dat", "3024.dat"}  # 1x1 브릭, 1x1 플레이트


def remove_1x1_bricks(ldr_path: str) -> int:
    """
    LDR 파일에서 1x1 브릭을 제거합니다.
...
def count_ldr_parts(ldr_path: str) -> int:
    """LDR 파일의 부품 수를 계산합니다."""
    count = 0
    with open(ldr_path, "r", encoding="utf-8") as f:
        for line in f:
