...
- 퍼널 단계별 분포: {funnel}
- 유저 마찰 지점 (Exit Points): {exits}
- 엔진 품질 및 비용: {quality}

[분석 및 제안 가이드]
1. '지표 이면의 맥락': 현재 유저들이 가장 몰입하고 있는 기능이나 콘텐츠 테마가 무엇인지 데이터로 설명하세요.
2. '잠재적 위험/기회': 지표는 정상이지만, 서서히 변하고 있는 태그 트렌드나 특정 시간대 유저 이탈 징후가 있는지 검토하세요. 특히 퍼널 이탈 지점을 분석하여 UX 마찰을 찾아내세요.
...
마크다운 형식을 적극 활용하여, '지표 기반 인사이트' → '퍼널 및 유저 행동 분석' → '성장 액션 제안'의 흐름으로 작성하세요.

다음 JSON으로만 응답하세요:
{{"report": "종합 운영 인사이트 보고서 내용 (마크다운 형식)"}}"""
...
2. 퍼널 및 이탈 분석 반영: 제품 인텔리전스 데이터를 활용하여 유저가 어느 지점에서 어려움을 겪는지(Exit Points) 구체적으로 언급하세요.
3. 전문가적 추론 (Heuristic): 데이터가 부족할 경우, 풍부한 분석 경험을 바탕으로 전문가적인 추측을 더하되 확신 수준을 명시하세요.
4. 운영 및 보고서 개선 제안: 질문이 서비스 개선이나 보고서 수정과 관련되어 있다면, 새로운 지표 수집 관점이나 GA4 맞춤 정의 항목을 구체적으로 제안하세요.
5. 가독성: 마크다운 형식을 적극 활용하여 전문적이고 깔끔하게 작성하세요.
