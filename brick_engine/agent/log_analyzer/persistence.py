...
async def archive_job_logs(job_id: str, status: str = "FAILED", container_name: str = "brickers-ai-container"):
    """
    ...
    try:
        ...
        # Job ID가 포함된 로그만 필터링 (RUNNING 상태일 때는 시작 부분만 있을 수 있음)
        job_logs = [line for line in raw_logs.splitlines() if job_id in line]
        
        if not job_logs and status != "RUNNING":
            ...
            full_log_text = "\n".join(job_logs) if job_logs else f"[{status}] Job Started"
