...
- 이전에 실패한 전략이라도 문제에 가장 적합하면 다시 선택할 것
- 과거 교훈은 "어떤 전략을 피할지"가 아니라 "어떻게 더 잘 실행할지" 참고용"""
...
- ⚠️ 각 제안은 모델의 서로 다른 위치/부품을 대상으로 할 것. 같은 위치에 대한 변형 반복 금지
...
lesson 형식 (반드시 준수):
"[전략명] 부유 X→Y(±Z), 충돌 X→Y(±Z), 품질 X→Y(±Z). [실패/성공 원인 1줄]"

금지 (위반 시 무효 처리):
- "다음에는 다른 접근 방식을 고려해야 한다" 류의 추상적 표현
- "효과적이지 않았음을 확인했다" 류의 동어반복
- "품질 향상을 도모해야 한다" 류의 공허한 결론
- 수치 변화 없이 감상만 쓰는 것

예시:
✅ "대칭수정 부유 26→26(±0), 품질 50→50(±0). 추가한 3개 브릭이 전부 충돌로 배치 실패"
✅ "위치이동 부유 12→8(-4), 품질 40→55(+15). 앞다리 2개를 1스터드 아래로 이동해서 접지 성공"
❌ "이번 진화에서 대칭 수정 전략이 효과적이지 않았음을 확인했다. 다른 방식을 고려해야 한다."
❌ "대칭 수정 외에 다른 요소들이 품질에 미치는 영향을 분석하는 것이 중요하다."
